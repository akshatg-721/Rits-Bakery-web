...
export function SocialLinks({
  ...
  return (
    <div className={cn('flex flex-col items-center gap-4 sm:items-end', className)}>
      <p
        className={cn(
          'text-xs font-semibold uppercase leading-relaxed tracking-[0.18em] text-gray-400',
          labelClassName,
        )}
      >
        {label}
...
export function Newsletter({ socialOnly = false }: NewsletterProps) {
  ...
  if (socialOnly) {
    return (
      <section
        ...
        className="overflow-hidden bg-[#FAFAF8] px-4 py-8 sm:px-6 lg:px-8"
      >
        <div className="mx-auto flex max-w-sm justify-center">
          <SocialLinks
            className="items-center sm:items-center"
            label="FOLLOW US"
            labelClassName="font-bold tracking-widest text-[#111111]"
          />
        </div>
      </section>
    )
        ...
        <div className="w-full sm:max-w-md">
          ...
          {submitted ? (
            ...
            <form
              ...
              className="mt-5 flex min-h-14 items-center rounded-full border border-gray-200 bg-[#FAFAF8] px-4 py-1 shadow-[0_4px_20px_rgb(0,0,0,0.04)] transition-all duration-200 focus-within:border-[#006241]/40 focus-within:bg-white focus-within:ring-4 focus-within:ring-[#006241]/10"
            >
              <input
                type="email"
                required
